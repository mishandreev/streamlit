...
def load_data() -> pd.DataFrame:
    ...
    materials['Тип ресурса'] = 'Материал'
    equipment['Тип ресурса'] = 'Оборудование'
    mechanisms['Тип ресурса'] = 'Механизм'
...
with tab2:
    ...
    if not filtered_df.empty:
        st.header("📈 Аналитика")
        tab1, tab2 = st.tabs(["Распределение по типам", "Параметры"])

        with tab1:
            fig, ax = plt.subplots()
            filtered_df['Тип ресурса'].value_counts().plot.pie(autopct='%1.1f%%', ax=ax)
